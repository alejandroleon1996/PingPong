...
public class JuegoDificil implements Initializable{
	...
	private void run(GraphicsContext gc) {
		...
		if(empezar) {
			...
			// La Superpoderosa IA
			if(posicionBolaX < ancho - ancho  / 17) {
		...
		if( ((posicionBolaX + bola > J2posicionX) && posicionBolaY >= J2posicionY && posicionBolaY <= J2posicionY + alturaJugador) || 
			...
			velocidadY = 5.5 * Math.signum(velocidadY);
			velocidadX = 5.5 * Math.signum(velocidadX);
			velocidadX *= -1;
			velocidadY *= -1;
		}
